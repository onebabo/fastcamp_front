...
/************** 강좌 공통함수 끝 ***********************************/
  ...
  const timetable_fetch = remote('timetable/fetch/', {
    result: function ({ result, classroom, chapters }) {
      const obj = {}
      const { classid } = _.first(classroom)
      obj[classid] = chapters
      return result === 200 && obj
    },
    store: function (v) {
      
      return store['timetables'] = _.extend({}, store['timetables'] || {}, v)
    }
  })

  const fetch_timetable = _.pipe(
    function ({ rows }) {
      return rows
    },
    _.map(function ({ courses }) {
      return _.map(courses, function ({ classid }) {
        return timetable_fetch.fetch({ classid })
      })
    }),
    function () {
      console.log(store['timetables'])
    }
  )
